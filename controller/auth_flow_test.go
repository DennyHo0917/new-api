...
func TestPasswordRegistrationAlwaysRequiresAndConsumesEmailVerification(t *testing.T) {
	for _, tc := range []struct {
		name, path, username, email string
		handler                     gin.HandlerFunc
	}{
		{"standard", "/api/user/register", "standard-user", "standard@example.com", Register},
		{"distribution", "/api/dist/user/register", "distribution-user", "distribution@example.com", DistRegister},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			withoutCode := fmt.Sprintf(`{"username":%q,"email":%q,"password":"verified-password"}`, tc.username, tc.email)
			response := securityEnrollmentRequest("POST", tc.path, withoutCode, "", service.AuthIdentity{}, tc.handler)
			assert.Contains(t, response.Body.String(), `"success":false`)

			common.RegisterVerificationCodeWithKey(tc.email, "123456", common.EmailVerificationPurpose)
			withCode := fmt.Sprintf(`{"username":%q,"email":%q,"password":"verified-password","verification_code":"123456"}`, tc.username, tc.email)
			response = securityEnrollmentRequest("POST", tc.path, withCode, "", service.AuthIdentity{}, tc.handler)
			assert.Contains(t, response.Body.String(), `"success":true`)
			assert.False(t, common.VerifyCodeWithKey(tc.email, "123456", common.EmailVerificationPurpose))

			var user model.User
			require.NoError(t, model.DB.Where("username = ?", tc.username).First(&user).Error)
