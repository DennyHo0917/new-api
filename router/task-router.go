...
func SetTaskRouter(router *gin.Engine) {
	...
	taskSubmitRouter.Use(middleware.RouteTag("relay"), middleware.DistDualTrackGateway(), middleware.TokenAuth())
	{
	...
	taskReadRouter.Use(middleware.RouteTag("relay"), middleware.DistDualTrackGateway(), middleware.TokenAuth())
	{
