...
- Production backend images MUST be built by the existing GitHub Actions deployment workflow after changes are pushed to `main`; agents MUST NOT compile or build Docker images on the VPS. The workflow transfers the built image archive to the VPS, where deployment is limited to `docker load` and starting the container with the existing Compose configuration. Do not replace this with a VPS-side `docker build`; a registry-based `docker pull` flow may be introduced only when the deployment workflow is explicitly changed to publish images to a registry.
