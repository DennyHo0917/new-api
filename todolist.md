...
- [x] 后端每日同步当前渠道全部模型的 BaseLLM 官方原生厂商价格，峰谷模型固定采用官方峰值价；模型广场按“官方价 × 分组倍率”展示，并补齐 DeepSeek `-0731` / `-0813` 版本别名映射（定向 Go 回归、全量 Go 构建、前端价格解析检查及生产构建通过，2026-09-19）
- [x] 定价页接入后端模型广场数据，并移除模型列表静态兜底；已按 New API 模型广场改为全宽左侧固定筛选、右侧彩色厂商 Logo 模型卡片，并补充分组价格对比、价格详情、模型简介/标签/厂商/端点展示；未配置基础模型价格时明确提示，不再展示 37.5 兜底倍率计算出的假价格（前端构建及浏览器验证通过）
