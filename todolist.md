...
- [ ] **5.1 本地联调测试**
  ...
  - [ ] 2026-09-17 后端四包全量测试未通过：service 出现渠道亲和缓存断言失败及 database is closed；不能据此宣称全量回归通过。
...
- [ ] **5.3 前端生产切流割接**
  - [ ] **正式割接硬门槛：切流前最后一次从旧站拉取完整用户清单，生成旧站用户迁移白名单快照，并核对总数、旧站 ID/用户名/邮箱唯一性及字段完整性；不得提前写入正式 `users` 表，用户首次登录验证成功后再按需创建本地账户。此项未完成不得触发生产切流。**
  - [ ] **正式割接硬门槛：新后端至少配置一个真实可用的生产上游渠道，完成凭据、模型映射、定价与额度策略配置，并使用本地新 Key 通过非流式、流式真实调用及扣费核对；同时验证上游不可用时不会错误扣费或回退到旧用户余额。此项未完成不得触发生产切流。**
  ...
  - [ ] 将 `apiroute.subrouter.ai` 配置为新后端的普通上游渠道，验证新后端用户、Key、计费与零垫资边界
    ...
    - [ ] 支持在单个渠道内按模型分配路由分组，并在渠道编辑界面维护 SubRouter 的模型分组（实现及 SQLite 定向回归完成；MySQL/PostgreSQL 实库验证待补）
