...
- [ ] **5.1 本地联调测试**
  ...
  - [x] 2026-09-20 新增只读一键切流前检查 `scripts/cutover-preflight.sh`：检查 Compose、三容器健康、核心环境变量存在性、Session Secret、Secure Cookie/可信 Origin、本地状态与 SubRouter 分发入口、PG15 版本与用户数、磁盘、24 小时内数据库备份、后端镜像和 Nginx 配置；不输出密钥值，任一硬门槛失败即返回非零。VPS 实测 14 项通过，当前仅 Secure Cookie/可信 Origin 和新鲜数据库备份两项未完成。
